...
public class EventServiceImpl implements EventService {
    ...
    public EventFullDto updateEventByAdmin(UpdateEventAdminRequest request, Long eventId) {
        ...
        LocationDto location = checkAndSaveLocation(request.getLocation());
        ...
        if (AdminStateAction.PUBLISH_EVENT.equals(request.getStateAction())) {
            if (event.getState().equals(EventState.PENDING)) {
                event = utilEventClass.updateEvent(event, request, category, locationMapper.toLocation(location));
    ...
    private long getViewsFromStats(Event event) {
        ...
        try {
            ...
            String start = event.getCreateOn().minusSeconds(1).format(formatter);
    ...
    private LocationDto checkAndSaveLocation(LocationDto newLocation) {
        if (newLocation == null) {
            return null;
        }
        Location location = locationRepository.findByLatAndLon(newLocation.getLat(), newLocation.getLon())
                .orElse(null);
        if (location == null) {
            return locationMapper.toLocationDto(locationRepository.save(locationMapper.toLocation(newLocation)));
        }
